...
We recorded and analysed a total of 400 videos from 80 lizards (n = 20 per treatment, see @fig_results). The lizards were between `r min(data_num$age_raw)` and `r max(data_num$age_raw)` days post-hatching (see Fig. S5 in _Supplementary Material_). Final sample sizes were composed by 43 females and 37 males, with no less than 6 females per treatment and no more than 13.               
