...
Two weeks before starting the experimen phase (see below), lizards were moved to the experimental arena for acclimatation. The arenas were individual medium size (41 L x 29.7 W x 22 H cm) plastic containers provided with a shelter (9 L x 6 W x 1.5 H cm) on one of the extremes and a water dish on the center (@fig-Methods B). The new enclosures were placed in 7 different racks located in two different rooms. Each rack was associated to its own CCTV systems (device model DVR-HP210475) allowing us to record lizards' behaviour during the experiment. The number of lizards per species and treatment in each rack was counterbalanced to control for any effect of the room or the position of the lizard in the rack. During acclimatation and throughout the experiment, lizards were fed with only one cricket per day dusted with calcium and multivitamin (see protocol below), and water was supplied *ad libitum*. The temperature of both rooms was set to between 22-24ºC, but we also provided a temperature gradient by means of a heat cord and heat lamps in a 12 h light: 12 h dark cycle.  

The numerical discrimination test consisted in a series of two-choice trials where lizards where presented with different number of food items. Each choice was placed in one of two sides of a white 3D printed platform (see @fig-Methods B, C) and then covered with a transparent Petri dish to prevent the lizard from obtaining the food. We performed five tests that differed in the ratio between both choices: 1 VS 4, 1 VS 3, 2 VS 4, 2 VS 3, 3 VS 4. We never used more than four items because higher numbers are considered to be encoded by different regions of the brain [**REFs**]. The food consisted in frozen small crickets (*A. domestica*) that were thawed and dusted with calcium and multivitamin. We decided to use frozen crickets to avoid movement to affect lizards' behaviour. Lizards were habituated to the frozen crickets by providing them with one per day during one month before the beginning of the experiments. During the acclimatation period, to habituate lizards to the platform and the Petri dish, the frozen cricket that was placed on top of the Petri dish in one of the sides of the platform, simulating the conditions of the experiment (see @fig-Methods B). The side where the food was placed was changed every day to avoid any side bias during the trials. The side where we placed the higher number of crickets was randomised between trials and individuals. We also counterbalanced the amount of individuals per treatment tested to the same type of trial everyday. The crickets used during the experiment were all the same size, but to control for other quantitative traits [**REFs**], we changed the orientation of the crickets as shown in @fig-Methods D. In addition, after the experiments, we made another set of trials to test whether the orientation of the crickets could affect lizards' choice, but we found no effect of orientation (see Supplementary Material).  

All tests were done between 10 and 12 in the morning when the lizards werew most active. Each trial was performed daily for five days, between the 17^th^ to the 22^nd^ of May 2024. After the tests, videos were analysed manually using (**PROGRAM**). We recorded two main variables: the first choice of the lizard, and the interest shown for the higher amount of food. The first choice was obtained after the first interaction with one of the two options: 1 if they interacted with the higher number of items first, 0 if not. For us to determine the first choice, we considered that the lizard was interacting with the food when it touched the Petri dish with its snout or when the front half of the body was on top of the platform. For five minutes after the first interaction with each option, we recorded the time spent interacting with it; we did this for both amounts of food independently of what was the first choice. We measured the interest shown for the higher amount of food by substracting the time spent (for a max of 5 min) in the platform with less crickets to the time spent in the platform with more crickets. This gave as a gradient from -300 to +300 where the minimum represents interest in the lower amount of food and the maximum shows a preference towards higher number of crickets, while 0 shows no discrimination between them. If there was no interaction with one of the options during the test, we considered the time spent as 0 for that option. When there was no interaction with any of the choices, both choice and interest were noted as NA. We excluded from the analyses all the individuals that did not participate in less than three of the tests.   
...
summary_table <- data_num %>%
  filter(choice == 1) %>% # Filter to keep only correct choices
  group_by(trt, test_type) %>% # Group by treatment and test
  summarise(count = n()) %>% # Count the number of correct choices
  pivot_wider(names_from = test_type, values_from = count, values_fill = list(count = 0)) %>% # Spread tests to columns
data.frame()
write.csv(summary_table, file= "./output/Checking/summary_table.csv")
data_num_bias <- data_num %>%
  mutate(
    actual_choice_lat = case_when(
      pos_high_lat == "Right" & choice == 1 ~ "Right",
      pos_high_lat == "Right" & choice == 0 ~ "Left",
      pos_high_lat == "Left" & choice == 1 ~ "Left",
      pos_high_lat == "Left" & choice == 0 ~ "Right",
      TRUE ~ NA_character_
    ),
    actual_choice_dist = case_when(
      pos_high_risk == "Close" & choice == 1 ~ "Close",
      pos_high_risk == "Close" & choice == 0 ~ "Far",
      pos_high_risk == "Far" & choice == 1 ~ "Far",
      pos_high_risk == "Far" & choice == 0 ~ "Close",
      TRUE ~ NA_character_
    )
  )
summary_biases <- data_num_bias %>%
  group_by(trt) %>% # Group by treatment and test
  summarise(
    right_choices = sum(actual_choice_lat == "Right", na.rm = TRUE),
    left_choices = sum(actual_choice_lat == "Left", na.rm = TRUE),
    close_choices = sum(actual_choice_dist == "Close", na.rm = TRUE),
    far_choices = sum(actual_choice_dist == "Far", na.rm = TRUE)
  ) %>% # Count the number of correct choices
data.frame()
write.csv(summary_biases, file= "./output/Checking/summary_biases.csv")
```
